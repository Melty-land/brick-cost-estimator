...
async function main() {
  ...
  // ---- 1. 初始值 ----
  const init = [['B34', '2,428,000'], ['H34', '2428'], ['H33', '5'], ['K17', '1,000,000'], ['E6', '100,000']];
  for (const [a, e] of init) {
    const t = await cellText(a);
    if (t !== e) { console.error(`✗ 初始 ${a} 应为 ${e},实得 ${t}`); process.exit(1); }
  }
  console.log('✓ 初始值正确:成本总价①=2,428,000、②=2,428、模数=5、黑水泥用料金额=1,000,000、配比金额=100,000');
  ...
  if (v.e5 !== '120,000' || v.k16 !== '1,200,000' || v.b33 !== '2,628,000' || v.h33 !== '2628' || v.f16 !== '600') {
    console.error('✗ 单价联动失败:' + JSON.stringify(v)); process.exit(1);
  }
  console.log('✓ 单价联动:配比金额 100,000→120,000、用料金额 1,000,000→1,200,000、①2,428,000→2,628,000(②=2,628)、清单单价 F17 同步 600');
  ...
  if (v2.j16 !== '3000' || v2.b33 !== '3,482,000' || v2.h33 !== '3482' || v2.j27 !== '14,130') {
    console.error('✗ 锅数联动失败:' + JSON.stringify(v2)); process.exit(1);
  }
  console.log('✓ 锅数联动:黑水泥用量 2,000→3,000 公斤、①2,628,000→3,482,000、②2,428→3,482、总用量 14130 公斤');
