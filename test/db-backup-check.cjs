 ...
 * 运行:node test/db-backup-check.cjs(依赖服务在跑)
 */
'use strict';
const fs = require('fs');
const os = require('os');
...
async function main() {
  // 1) 导出 zip
  const exp = await fetch(APP + 'api/db/export', { method: 'POST' });
  ...
  // 2) 记录当前数据快照(用于回灌后对比)
  const before = await (await fetch(APP + 'api/data')).json();
  ...
  const put = await fetch(APP + 'api/data', {
    method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(modified)
  });
  if (!put.ok) throw new Error('PUT HTTP ' + put.status);
  const afterPut = await (await fetch(APP + 'api/data')).json();
  ...
  // 5) 校验数据恢复为导出时状态
  const after = await (await fetch(APP + 'api/data')).json();
